...
// Delete related checkout items first to maintain referential integrity
if ($koneksi) {
    if ($delItems = $koneksi->prepare('DELETE FROM checkout_item WHERE order_id = (SELECT order_id FROM checkout WHERE id_checkout = ?)')) {
    ...
    if ($delCheckout = $koneksi->prepare('DELETE FROM checkout WHERE id_checkout = ?')) {
